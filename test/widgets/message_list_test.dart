...
void main() {
  ...
  group('OutboxMessageWithPossibleSender', () {
    ...
    Finder messageNotSentFinder = find.text(
      'MESSAGE NOT SENT').hitTestable();
    ...
    testWidgets('[sendMessage] request reaches wait period time limit, retrieve the content to compose box, then message event arrives', (tester) async {
      await setupMessageListPage(tester,
        narrow: eg.topicNarrow(stream.streamId, 'topic'), streams: [stream],
        messages: []);
      await sendMessageAndSucceed(tester,
      ...
      // The [sendMessage] request succeeds; there is no outbox message affected.
